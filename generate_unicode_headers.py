...
def find_case_partner(cp: int) -> Tuple[Optional[int], str]:
    ...
    try:
        ...
        # current_name is not strictly needed but kept for completeness/debugging
...
def emit_header(block_name: str,
    ...
    FIXED: Uses a robust naming convention for all files and updates the comment.
    """
    # 1. Implement robust sanitization for file name (Addressing Request 2)
    ...
    # Do not use the hex fallback naming (e.g., 'block_1AFF0')
    header_file = out_dir / f"{file_basename}.h"

    # Define the boilerplate lines
    # 2. Use the final filename in the comment (Addressing Request 1)
    lines: List[str] = [
    ...
    # Only write the file if defines were added
